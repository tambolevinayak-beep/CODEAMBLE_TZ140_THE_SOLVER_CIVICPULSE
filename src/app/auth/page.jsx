...
export default function Authentication() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Auth is disabled, just redirect to citizen dashboard
    router.push('/citizen');
  };
...
<button onClick={handleSubmit} className="w-full flex items-center justify-center gap-md py-sm px-lg bg-surface border border-outline-variant rounded-lg font-label-md text-label-md text-on-surface hover:bg-surface-container-low transition-colors duration-200">
...
<input className="w-full bg-surface border border-outline-variant rounded-lg px-md py-sm font-body-md text-body-md text-on-surface focus:outline-none focus:border-primary-container focus:ring-2 focus:ring-primary-container/20 transition-all placeholder:text-outline" id="password" placeholder="••••••••" required={!isMagicLink} type={showPassword ? 'text' : 'password'}/>
...
<button className="mt-md w-full bg-primary-container text-white font-label-md text-label-md py-sm px-lg rounded-lg shadow-sm hover:bg-primary hover:-translate-y-[1px] transition-all duration-200 flex justify-center items-center gap-sm" type="submit">
